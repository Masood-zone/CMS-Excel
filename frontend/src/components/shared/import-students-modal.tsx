...
export function ImportStudentsModal({ isOpen, onClose, onImport, classId }: ImportStudentsModalProps) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      const isValidType = validTypes.some(
        (type) => selectedFile.type === type || selectedFile.name.toLowerCase().endsWith(type),
      )
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportResult({
        ...
        totalRows: 0,
        successfulImports: 0,
        errors: [{ row: 0, field: "file", value: "", message: "Failed to process file" }],
        duplicates: [],
      })
    } finally {
  ...
  const downloadTemplate = () => {
    ...
    const csvContent = "name,age,gender\nJohn Doe,15,male\nJane Smith,14,female\nSample Student,16,male"
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel file (.xlsx or .xls) containing student data. The file should have columns for name, age,
            and gender.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
            <div>
              ...
              <p className="text-xs text-blue-700">Download our sample template to get started</p>
            </div>
            <Button
              variant="outline"
              size="sm"
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <p className="text-lg font-bold text-green-700">{importResult.successfulImports}</p>
                </div>
              ...
              {importResult.duplicates.length > 0 && (
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>Duplicate students found:</strong> {importResult.duplicates.join(", ")}
                    <br />
                    These students were skipped as they already exist in the system.
                  </AlertDescription>
                </Alert>
              )}
