...
export const recordController = {
  ...
  getStudentRecordsByClassAndDate: catchAsync(async (req: Request, res: Response) => {
    const classId = Number.parseInt(req.params.classId)
    const date = new Date(req.query.date as string)
    ...
    const records = await recordService.getStudentRecordsByClassAndDate(classId, date)
    res.status(200).json(records)
  }),

  getAllTeacherSubmittedRecords: catchAsync(async (req: Request, res: Response) => {
    const date = new Date(req.query.date as string)
    if (isNaN(date.getTime())) {
  ...
  getTeacherSubmittedRecords: catchAsync(async (req: Request, res: Response) => {
    ...
    const records = await recordService.getTeacherSubmittedRecords(teacherId, date)
    res.status(200).json(records)
  }),
  ...
  updateStudentStatus: catchAsync(async (req: Request, res: Response) => {
    ...
    const updatedRecord = await recordService.updateStudentStatus(Number.parseInt(id), { hasPaid, isAbsent })
    res.status(200).json(updatedRecord)
  }),

  update: catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params
    const updatedRecord = await recordService.updateRecord(Number.parseInt(id), req.body)
